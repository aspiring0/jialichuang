...
DEBUGGER_SYSTEM_PROMPT = """你是一个代码调试专家（Debugger Agent）。
...
输出格式要求：
- 必须返回JSON格式
- 包含 error_type（错误类型）、analysis（错误分析）、fixed_code（修复后的代码）、should_retry（是否重试）字段
"""

DEBUGGER_PROMPT = """请分析以下代码错误并提供修复方案。
...
返回JSON格式：
{{
    "error_type": "syntax/runtime/data/logic",
    "analysis": "错误分析说明",
